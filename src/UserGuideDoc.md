...
- 其他群里面评论或交流都 `看不见`，问题请反馈给提供的 `专门反馈渠道` ，反馈给别人没有用。
...
- 微信版本 `8.0.54`、`8.0.55`
- 主题盒子 `1.0.5` 以上
  （如遇到点击部分菜单或者界面闪退问题，请更新 `Themebox` 插件到 `1.1.1` 以上版本。）
<br>


> 其他微信版本 `未做适配`，部分图标显示错误和丢失属正常现象。<br>（不考虑向下兼容，请自行适配优化，如你愿意分享针对 `31、33` 版本的适配包，可以PR或私信提交分享，你将成为本项目的贡献者）

<br><br>

# 2. 如何安装本主题？

## 1. 通过上线版 `兑换码` 兑换本主题的安装方法（推荐）
> 使用 `Themebox` 兑换码兑换的请直接通过 `Themebox` -右上方的 `空投箱` 下载, 下载后启用本主题即可，详细使用配置请参照第四章节。
> 已有兑换码还未兑换？前往 [Themebox主题兑换](https://theme.25mao.com/)进行兑换。
...
但如需使用提供的 `自定义替换素材`，仍需要手动前往仓库下载主题包解压后将 `自定义替换文件夹` 内你所需要替换的素材导入到 `DIY` 文件夹) <br>
...
> <br>（兑换码 `不售卖`。即使没有兑换码你也仍然可以通过加载原生主题DIY素材的方式使用本主题）

<br><br>

## 2. 通过其他作者的 `原生主题` 加载DIY素材安装的方法
...
# 3. 安装主题不生效怎么办？
## 1. 如果你是通过其他作者的 `原生主题` 加载DIY素材，文件夹你拷对了吗？
...
### 2.6. 配色管理
> 更新：
> - 自`Themebox1.1.1`版本开始，`配色管理`可以单独设置红包转账字体颜色。设置为`白色`以兼容本主题红包转账卡面。
...
> 如果设置了怎么办？
> 如已设置，请前往 `配色管理` 设置 `红包转账封面字体颜色` 为 `白色`。
...
[PSD文件](https://github.com/AidenYang1/iMessageApp_WeChat_For_Themebox/tree/a7f92e918b155d8d658a09a3ce96b434b0aceaf9/%E5%85%B6%E4%BB%96/02%E7%BA%A2%E5%8C%85%E8%BD%AC%E8%B4%A6%E6%B0%94%E6%B3%A1%E5%88%B6%E4%BD%9C%EF%BC%88PS%E9%A1%B9%E7%9B%AE%E6%96%87%E4%BB%B6%EF%BC%89)
...
适配过程中 `采用过全彩` 的图标设计方案，实际界面感受是花里胡哨的，导致聊天界面的彩色要素太多，违背了主题基于系统APP设计的 `简约高效` 风格的理念。<br>
...
自`主题1.2.3`版本开始，顶栏设计移除了全局界面下的深色分隔线，仅保留聊天页面顶栏底部的分割线，并做出 `不透明度` 降低的优化。
...
<br><br><br>
# 12. 已知问题收集
在主题适配过程中，会由于 `主题插件` 、`微信团队` 设计等限制，导致部分内容和区域无法适配和优化细节。现将所有已知的问题列下。如有另外解决方法，可以issue提起解决方案，但此类问题不应再在反馈issue中当做问题提起。
  
  - 聊天页语音消息播放图标无法单独设置。自WeChat `8.0.55` 版本开始，聊天页语音消息 `我方`、`对方` 改用了逻辑代码水平翻转、`三状态图标` 图层叠加显示为最终未播放状态图标的设计。自主题 `1.2.2` 版本开始，弃用了 `8.0.54` 版本的双方播放条单独播放的设计，如需要请回滚版本到 WeChat `8.0.54`。
  - WeChat内浏览器访问网页前进按钮错位。
  - 小程序勾选记住状态、好友详情添加电话号码`加号图标`拉伸。（可自行改用svg素材加载）
  - 公众号右上角搜索为黑。
  - 小程序更多操作的 `问题反馈` 无法适配。
